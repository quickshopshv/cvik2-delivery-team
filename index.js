// Database-Free Delivery Bot - Updated for Driver Group Management
// Run in Deno

const TELEGRAM_BOT_TOKEN = Deno.env.get('TELEGRAM_BOT_TOKEN');
const DRIVER_GROUP_ID = -2806563117; // Your driver group chat ID
const ADMIN_USER_IDS = [5186573916]; // Your admin user IDs; add more if needed
...
async function showConnectedDrivers(chatId) {
  ...
  // Ideally you store driver names, but here just IDs shown
...
async function confirmOrder(orderNumber, chatId, messageId) {
  ...
  if (!order.customerId || !order.location) {
    await answerCallbackQuery('', 'Please set Customer ID and Location first!');
    return;
  }
  if (connectedDrivers.size === 0) {
...
async function handleDriverGroupMessage(message) {
  ...
  // Send inline keyboard on /start or commands if message is /start without parameters or other commands
  if (!text || text === '/start') {
  ...
  // For other messages exchange, you may ignore or give help message
}
...
async function handleChatMemberUpdate(update) {
  ...
  if (newStatus === 'left' || newStatus === 'kicked') {
    ...
    // Optionally notify admins driver joined/re-joined group
    for (const adminId of ADMIN_USER_IDS) {
...
Deno.serve(async (req) => {
  ...
  // GET for webhook setup
  if (req.method === 'GET') {
    const url = new URL(req.url);
    if (url.searchParams.get('setup')) {
      const webhookUrl = `https://yourdomain.com/your-webhook-path`; // Replace with your actual URL
  ...
  try {
    ...
    // Chat member updates for monitoring driver group membership
    ...
    if (update.message) {
      ...
      // Process driver group messages separately
      if (chatId === DRIVER_GROUP_ID) {
        await handleDriverGroupMessage(message);
        return new Response('OK', { headers: corsHeaders });
      }

      // Admin private chat commands and order edits
      if (isAdmin(userId)) {
        ...
        // Check if admin is in the middle of editing an order
      ...
      // Drivers can issue order status updates in private chat via command syntax:
      // /pickedup 0001, /arrived 0001, /completed 0001 - you can customize as needed
      ...
      // Customer or anyone else sending 4-digit order number to track order
    ...
    if (update.callback_query) {
      ...
      if (isAdmin(userId)) {
        ...
        // For non-admin users clicking on callbacks in admin panel - just acknowledge
        await answerCallbackQuery(update.callback_query.id);
      }
    }
